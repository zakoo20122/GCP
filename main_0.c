...
//选择父本
void choose_parents(int *parent_num,int *parents)
{
    //随机选择两个父本
    *parent_num=2;
...
void write_file(void)
{
    ...
    fprintf(f_run_data,"%d %s %d %.3f %d %04d%02d%02d%02d%02d\n",THE_CHOICE,instances[THE_CHOICE],num_color,run_time,num_cross,1900+p_time_struct->tm_year,1+p_time_struct->tm_mon,p_time_struct->tm_mday,p_time_struct->tm_hour,p_time_struct->tm_min);//类型(choice)、文件名、颜色数、运行时间、交叉次数、日期时间
    fclose(f_run_data);
}
//对解进行检查，输入解，输出结果
...
{
    int parent_num=0;
    int *parents=(int *)malloc(sizeof(int)*PARENTS_CROSSOVER_NUM);
    ...
    for(i=0;i<TEST_NUM;i++){
        ...
        while(1){
            ...
            choose_parents(&parent_num,parents);
